      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-1">
          <div className="flex justify-between items-center">
            <label className={labelClasses}>Payment Description</label>
            <button 
              type="button"
              onClick={handleSuggestDescription}
              disabled={suggesting || !formData.description}
              className="text-xs font-medium text-indigo-600 hover:text-indigo-800 disabled:text-slate-400 flex items-center gap-1 transition-colors mb-1.5"
            >
              {suggesting ? 'Refining...' : 'AI Refine'}
            </button>
          </div>
          <input 
            type="text" 
